    ...
    loop do
      ...
      for i in 0...pkmn.moves.length
        ...
        if !pkmn.egg? && (isConst?(move.id,PBMoves,:MILKDRINK) ||
                          ...
                          isConst?(move.id,PBMoves,:RECOVER) ||
                          isConst?(move.id,PBMoves,:AROMATHERAPY) ||
                          isConst?(move.id,PBMoves,:WISH) ||
                          isConst?(move.id,PBMoves,:SYNTHESIS) ||
                          isConst?(move.id,PBMoves,:HEALINGWISH) ||
      ...
      for i in 0...4
        if cmdMoves[i]>=0 && command==cmdMoves[i]
          ...
          if isConst?(pkmn.moves[i].id,PBMoves,:SOFTBOILED) ||
            ...
            pbRefresh
            break          elsif isConst?(pkmn.moves[i].id,PBMoves,:AROMATHERAPY)
            if pkmn.hp==0
              movename = PBMoves.getName(pkmn.moves[i].id)
            ...
            amt = [(pkmn.totalhp/5).floor,1].max
           elsif  isConst?(pkmn.moves[i].id,PBMoves,:SYNTHESIS)
            amt = [(pkmn.totalhp/2).floor,1].max
           elsif  isConst?(pkmn.moves[i].id,PBMoves,:WISH)
