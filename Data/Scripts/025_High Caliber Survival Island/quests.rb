           ...
           when QuestsData::NeededInfo["FailedMessage"]
             ...
             if matchData[1].include?("StageLocation")
               ...
               if record.is_a?(String) && record.downcase=="nil"
                 ...
                 Kernel.pbMessage(_INTL("{1}", record))
                 #Kernel.pbMessage(_INTL("5={1}", MessageTypes.getFromMapHashValue(MessageTypes::MapNames,"Route 1")))
...
class Messages
  def getFromMapHashValue(type,key)
    Kernel.pbMessage("Hi")
    delayedLoad
    #Kernel.pbMessage(_INTL("Hi2 {1}",@messages))
    return nil if !@messages
    #Kernel.pbMessage(_INTL("Hi3 {1}",@messages[0]))
    return nil if !@messages[0]
    #Kernel.pbMessage(_INTL("Hi4 {1}, {2}",@messages[0][type],@messages[0][0]))
    return nil if !@messages[0][type] && !@messages[0][0]
    Kernel.pbMessage("Hi5")
    key.sub!("e^","é")
