...
[:MASTERBALL,1,[:PURPLEAPRICORN,20],[:PINKAPRICORN,20], true]
]
...
# to the $game_variables[CRAFTVAR] array
    for i in 0...RECIPESMB.length
     $isUnlocked[i] = RECIPESMB[i][4]
    end
...
class PokeCrafterMBScene

  def initialize
    @close = $exit
    @select=3
    @item=0
    @mat1=RECIPESMB[@item][2]? RECIPESMB[@item][2][0] : -1 # the amount for first item
    @mat2=RECIPESMB[@item][3]? RECIPESMB[@item][3][0] : -1 # the amount for first item
    @cost1=RECIPESMB[@item][2]? RECIPESMB[@item][2][1] : 0 # the amount for first item
    @cost2=RECIPESMB[@item][3]? RECIPESMB[@item][3][1] : 0 # the amount for first item
    @amount=RECIPESMB[@item][1] # the amount for the first item made
    ...
    self.openPokeCrafterMBscene
  end
  
  def openPokeCrafterMBscene
  ...
  def closePokeCrafterMBscene
    ...
    def input
      ...
      when 3
        ...
        @sprites["Item_icon"].setBitmap(pbItemIconFile(getID(PBItems,RECIPESMB[@item][0])))
        if $game_variables[CRAFTVAR][@item]
            @sprites["unknown"].opacity=0
            @sprites["Item_icon"].opacity=255
            @sprites["Item_1_icon"].setBitmap(RECIPESMB[@item][2]? pbItemIconFile(getID(PBItems,RECIPESMB[@item][2][0])) : "") # Vendily
            @sprites["Item_2_icon"].setBitmap(RECIPESMB[@item][3]? pbItemIconFile(getID(PBItems,RECIPESMB[@item][3][0])) : "") # Vendily
            @sprites["Item_1_icon"].opacity= RECIPESMB[@item][2] ? 255 : 0
            @sprites["Item_2_icon"].opacity=RECIPESMB[@item][3] ? 255 : 0
            @mat1=RECIPESMB[@item][2]? RECIPESMB[@item][2][0] : -1
            @mat2=RECIPESMB[@item][3]? RECIPESMB[@item][3][0] : -1
            @cost1=RECIPESMB[@item][2]? RECIPESMB[@item][2][1] : 0
            @cost2=RECIPESMB[@item][3]? RECIPESMB[@item][3][1] : 0
            @amount=RECIPESMB[@item][1]
          else
        ...
        # When pressing Right
        if Input.trigger?(Input::RIGHT)  && @item < RECIPESMB.length-1
          @item+=1
        elsif Input.trigger?(Input::RIGHT)  && @item ==RECIPESMB.length-1 # Make it run though the selection after last item.
          @item = 0
        end
        ...
        when 2 
          if $game_variables[CRAFTVAR][@item]
            if $PokemonBag.pbQuantity(@mat1)<@cost1 || (@mat2!=-1 && $PokemonBag.pbQuantity(@mat2) <@cost2) #Seth Edited 
              ...
              $PokemonBag.pbStoreItem(RECIPESMB[@item][0],@amount)
  ...
  def text
    ...
    @text3=_INTL("{1} / {2}", @item + 1, RECIPESMB.length)
