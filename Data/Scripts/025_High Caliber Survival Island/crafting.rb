...
class ItemCraft_Scene
  ...
  BASELIGHT        = MessageConfig::LIGHTTEXTBASE
  SHADOWLIGHT      = MessageConfig::LIGHTTEXTSHADOW
  BASEDARK         = MessageConfig::DARKTEXTBASE
  SHADOWDARK       = MessageConfig::DARKTEXTSHADOW
  ...
  def pbCraftItem(stock)
    ...
    loop do
      ...
      if Input.trigger?(Input::C)
        item = @stock[index][0]
        itemname = (volume>1) ? PBItems.getNamePlural(item) : PBItems.getName(item)
  ...
  def disposeIcons(index)
    num = @stock[index][1].length/2
    num.times do |i|
      @sprites["ingredient_#{i}"].dispose
    end
  end
  ...
  def refreshNumbers(index,volume)
    ...
    num = @stock[index][1].length/2
    textpos = []
    textpos.push([_INTL("x{1}",volume),@w+26,@h+100,0,BASEDARK,SHADOWDARK])
    num.times do |i|
      ingredient = @stock[index][1][2*i]
  ...
  def pbRedrawItem(index,volume)
    ...
    @overlay1.clear
    item =  @stock[index][0]
    @sprites["item"].item = item
    @sprites["itemtext"].text = @adapter.getDescription(item)
    num = @stock[index][1].length/2
    textpos = [
    ["C: Craft",4,Graphics.height-32,0,BASELIGHT,SHADOWLIGHT],
    ["ARROWS: Navigate",Graphics.width/2,Graphics.height-32,2,BASELIGHT,SHADOWLIGHT],
    ["X: Exit",Graphics.width-4,Graphics.height-32,1,BASELIGHT,SHADOWLIGHT]
    ]
    textpos.push([@adapter.getName(item),@w+98,@h+6,0,BASEDARK,SHADOWDARK])
    num.times do |i|
      ingredient = @stock[index][1][2*i]
      quantity = @stock[index][1][2*i+1]
      @sprites["ingredient_#{i}"] = ItemIconSprite.new(0,0,ingredient,@viewport)
      @sprites["ingredient_#{i}"].x = @w + 38
      @sprites["ingredient_#{i}"].x += 186 if i>2
      @sprites["ingredient_#{i}"].y = @h + 198 + (i%3)*48# Graphics.height/2 + @w + (i%3)*48
      textpos.push([@adapter.getName(ingredient),
      @xPos[i/3],
      @yPos[i%3],
...
def pbItemCrafter(stock,speech1=nil,speech2=nil)
  for i in 0...stock.length
    ...
    stock[i][0] = getID(PBItems,stock[i][0])
    for j in 0...stock[i][1].length/2
      k = 2*j
      stock[i][1][k] = getID(PBItems,stock[i][1][k])
    end
    if !stock[i][0] || stock[i][0]==0
      stock[i] = nil # If target item does not exist
    end
    if !stock[i].nil? && stock[i][1].include?(0)
