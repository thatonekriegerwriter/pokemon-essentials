...
#
# *to add an item of your own just add it to the RECIPESPB array.
...
RECIPESPB=[
[:POKEBALL,1,[:REDAPRICORN,1],nil, true],
[:GREATBALL,1,[:REDAPRICORN,2],[:BLUEAPRICORN,1], true],
[:ULTRABALL,1,[:BLACKAPRICORN,2],[:YELLOWAPRICORN,1], true],
[:DIVEBALL,1,[:PINKAPRICORN,1],[:BLUEAPRICORN,2], true],
[:DUSKBALL,1,[:BLACKAPRICORN,2],[:BLUEAPRICORN,1], true],
[:FASTBALL,1,[:REDAPRICORN,1],[:YELLOWAPRICORN,2], true],
[:FRIENDBALL,1,[:GREENAPRICORN,2],[:YELLOWAPRICORN,1], true],
[:HEALBALL,1,[:PINKAPRICORN,1],[:WHITEAPRICORN,2], true],
[:HEAVYBALL,1,[:BLUEAPRICORN,2],nil, true],
[:LEVELBALL,1,[:BLACKAPRICORN,2],[:REDAPRICORN,1], true],
[:LOVEBALL,1,[:WHITEAPRICORN,2],nil, true],
[:QUICKBALL,1,[:YELLOWAPRICORN,1],[:BLUEAPRICORN,2], true],
[:LUXURYBALL,1,[:REDAPRICORN,1],[:WHITEAPRICORN,2], true],
[:MOONBALL,1,[:BLACKAPRICORN,1],[:BLUEAPRICORN,2], true],
[:NESTBALL,1,[:GREENAPRICORN,2],[:YELLOWAPRICORN,1], true],
[:NETBALL,1,[:BLACKAPRICORN,1],[:BLUEAPRICORN,2], true],
[:PREMIERBALL,1,[:REDAPRICORN,1],[:WHITEAPRICORN,2], true],
[:REPEATBALL,1,[:REDAPRICORN,1],[:BLACKAPRICORN,2], true],
[:DREAMBALL,1,[:WHITEAPRICORN,2],[:PINKAPRICORN,1], true],
[:MOONBALL,1,[:YELLOWAPRICORN,2],[:BLACKAPRICORN,1], true],
[:TIMERBALL,1,[:WHITEAPRICORN,2],[:BLACKAPRICORN,1], true]
]

###############################################################################

# This goes through the RECIPESPB array and adds the true or false value from it
# to the $game_variables[CRAFTVAR] array
    for i in 0...RECIPESPB.length
     $isUnlocked[i] = RECIPESPB[i][4]
    end
...
class PokeCrafterScene

  def initialize
    @close = $exit
    @select=3
    @item=0
    @mat1=RECIPESPB[@item][2]? RECIPESPB[@item][2][0] : -1 # the amount for first item
    @mat2=RECIPESPB[@item][3]? RECIPESPB[@item][3][0] : -1 # the amount for first item
    @cost1=RECIPESPB[@item][2]? RECIPESPB[@item][2][1] : 0 # the amount for first item
    @cost2=RECIPESPB[@item][3]? RECIPESPB[@item][3][1] : 0 # the amount for first item
    @amount=RECIPESPB[@item][1] # the amount for the first item made
    ...
    @sprites["Item_icon"].setBitmap(pbItemIconFile(getID(PBItems,RECIPESPB[@item][0])))
    ...
    self.openPokeCrafterscene
  end
  
  def openPokeCrafterscene
    ...
    def input
      ...
      when 3
        ...
        if $game_variables[CRAFTVAR][@item]
            ...
            @sprites["Item_1_icon"].setBitmap(RECIPESPB[@item][2]? pbItemIconFile(getID(PBItems,RECIPESPB[@item][2][0])) : "") # Vendily
            @sprites["Item_2_icon"].setBitmap(RECIPESPB[@item][3]? pbItemIconFile(getID(PBItems,RECIPESPB[@item][3][0])) : "") # Vendily
            @sprites["Item_1_icon"].opacity= RECIPESPB[@item][2] ? 255 : 0
            @sprites["Item_2_icon"].opacity=RECIPESPB[@item][3] ? 255 : 0
            @mat1=RECIPESPB[@item][2]? RECIPESPB[@item][2][0] : -1
            @mat2=RECIPESPB[@item][3]? RECIPESPB[@item][3][0] : -1
            @cost1=RECIPESPB[@item][2]? RECIPESPB[@item][2][1] : 0
            @cost2=RECIPESPB[@item][3]? RECIPESPB[@item][3][1] : 0
            @amount=RECIPESPB[@item][1]
          else
        ...
        # When pressing Right
        if Input.trigger?(Input::RIGHT)  && @item < RECIPESPB.length-1
          @item+=1
        elsif Input.trigger?(Input::RIGHT)  && @item ==RECIPESPB.length-1 # Make it run though the selection after last item.
          @item = 0
        end
        ...
        when 2 
          if $game_variables[CRAFTVAR][@item]
            if $PokemonBag.pbQuantity(@mat1)<@cost1 || (@mat2!=-1 && $PokemonBag.pbQuantity(@mat2) <@cost2) #Seth Edited 
              ...
              $PokemonBag.pbStoreItem(RECIPESPB[@item][0],@amount)
              $PokemonBag.pbDeleteItem(@mat1,@cost1)
              if @mat2!=-1
                $PokemonBag.pbDeleteItem(@mat2,@cost2)
              end
              self.text
              Kernel.pbMessage(_INTL("{1} {2}'s were crafted.", @amount, PBItems.getName(getID(PBItems,RECIPESPB[@item][0]))))
            end
          else
  ...
  def text
    ...
    @text3=_INTL("{1} / {2}", @item + 1, RECIPESPB.length)
