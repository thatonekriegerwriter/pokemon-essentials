...
RECIPEMART1=[
[:POTION,1,[:STARPIECE,2],nil, true],
[:SUPERPOTION,1,[:STARPIECE,5],nil, true],
[:REPEL,1,[:STARPIECE,16],nil, true],
[:ETHER,1,[:STARPIECE,10],nil, true],
[:BLACKFLUTE,1,[:STARPIECE,20],nil, true],
[:ESCAPEROPE,1,[:STARPIECE,7],nil, true],
[:DUBIOUSDISC,1,[:STARPIECE,50],nil, true],
[:SACREDASH,1,[:STARPIECE,150],nil, true],
[:ATKCURRY,1,[:STARPIECE,25],nil, true],
[:SATKCURRY,1,[:STARPIECE,25],nil, true],
[:SPEEDCURRY,1,[:STARPIECE,25],nil, true],
[:SPDEFCURRY,1,[:STARPIECE,25],nil, true],
[:ACCCURRY,1,[:STARPIECE,25],nil, true],
[:DEFCURRY,1,[:STARPIECE,25],nil, true],
[:CRITCURRY,1,[:STARPIECE,25],nil, true],
[:GSCURRY,1,[:STARPIECE,25],nil, true],
[:PICKAXE,1,[:STARPIECE,50],nil, true],
[:POKEDOLL,1,[:STARPIECE,10],nil, true],
...
class TradingMart1Scene
  ...
  def initialize
    ...
    @sprites["Item_icon"].setBitmap(pbItemIconFile(getID(PBItems,RECIPEMART1[@item][0])))
    ...
    def input
      ...
      when 3
        ...
        if $game_variables[CRAFTVAR][@item]
            ...
            @sprites["Item_1_icon"].setBitmap(RECIPEMART1[@item][2]? pbItemIconFile(getID(PBItems,RECIPEMART1[@item][2][0])) : "") # Vendily
            @sprites["Item_2_icon"].setBitmap(RECIPEMART1[@item][3]? pbItemIconFile(getID(PBItems,RECIPEMART1[@item][3][0])) : "") # Vendily
        ...
        when 2 
          if $game_variables[CRAFTVAR][@item]
            if $PokemonBag.pbQuantity(@mat1)<@cost1 || (@mat2!=-1 && $PokemonBag.pbQuantity(@mat2) <@cost2) #Seth Edited 
              ...
              self.text
              Kernel.pbMessage(_INTL("{1} {2}'s were traded for each other.", @amount, PBItems.getName(getID(PBItems,RECIPEMART1[@item][0]))))
            end
          else
